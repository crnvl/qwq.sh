...
const Home: NextPage = () => {
	...
	return <>
		...
		<div className='flex w-screen h-screen bg-custom-400'>
			<div className={`
			flex flex-col md:flex-row justify-center m-auto gap-4
			`}>
				<input id='input' type="text" className="px-8 py-4 text-custom-400 font-bold rounded-md outline-none focus:shadow-lg transition ease-in-out duration-300" placeholder='Enter link here' />
				<button onClick={createShortUrl} className="flex justify-center px-8 py-4 font-bold rounded-md bg-black text-white shadow-lg hover:shadow-none transition ease-in-out duration-300 active:scale-95 active:bg-white active:text-custom-400">
					{icon}
				</button>
			</div>
		</div>
